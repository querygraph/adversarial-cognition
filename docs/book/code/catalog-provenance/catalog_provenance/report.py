"""Hard gates, metrics, and report assembly for CATALOG-PROVENANCE-v1.

Safety failures are counted in named hard gates that must all be zero; they are
never averaged into a quality score. A catalog is scored only on the
capabilities it declares — a case whose capability the catalog does not claim
is ``unsupported``, never a pass or a failure. Reports carry bounded IDs,
...
BENCHMARK = "CATALOG-PROVENANCE-v1"
MAX_REPORT_TEXT = 256
...
def hard_gates(case_outcomes: list[dict]) -> dict[str, int]:
    ...
    catalog got wrong — an honestly-declared unsupported case never trips it."""
...
def catalog_status(status: str, gates: dict[str, int]) -> str:
    if status != "executed":
        return status
    ...
    def walk(value: object) -> None:
        if isinstance(value, str):
            if len(value) > MAX_REPORT_TEXT or "\n" in value:
                raise ValueError("catalog-provenance report contains an unbounded string")
...
def build_report(metadata: dict, manifest_digest: str, catalogs: list[dict]) -> dict:
    reference = next((c for c in catalogs if c["catalog"] == "reference"), None)
    overall = "pass"
