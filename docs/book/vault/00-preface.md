![Plato disputing with Diogenes over the rug at an Academy symposium — the original adversarial gesture, and the frontispiece of a book about attacking a boundary on purpose.](../Evidence/trilogy-headboard.png)
...
This is the **vault edition**. Every benchmark the book describes is present here
in full, under `Evidence/` — the deterministic reference boundary, the shared
adapter contract, every competitor adapter, the catalog and capability suites,
and the performance harness. Where the text shows an excerpt, a *"Full source in
the vault"* link opens the exact file it was taken from. Read the argument in the
Reader; open the code beside it when you want to see the claim made good.
